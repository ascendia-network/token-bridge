import { stage, stageConfig } from "../configs/config";
import { logger } from "./logger";

const tokenLogger = logger.child({ service: "token" });
...
export async function getToken(network: string, tokenAddress: string): Promise<Token> {
    ...
    if (tokens[key] === undefined) {
        tokenLogger.warn("Token %s in %s network not found in config", tokenAddress, network);
