...
interface ITokenManager {
  ...
  function bridgableTokens(address token) external view returns (bool isBridgable);

  /// Get external token address
  /// @param token address of the token
  /// @return externalTokenAddress external token address
  function token2external(address token) external view returns (bytes32 externalTokenAddress);
  ...
  function external2token(bytes32 externalTokenAddress) external view returns (address token);
