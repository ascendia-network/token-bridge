...
contract SendTokens is DeployerBase {
    ...
    function getPayload(string memory path) public view returns (BridgeTypes.SendPayload memory) {
        try vm.readFile(path) returns (string memory json) {
            bytes memory data = vm.parseJson(json);
            SendPayloadJson memory payloadJson = abi.decode(data, (SendPayloadJson));
    ...
    function run(string memory path, bytes memory signature) public {
        ...
        bridge.send{value: payload.feeAmount}(
            bytes32(0x111122223333444455556666777788889999AAAABBBBCCCCDDDDEEEEFFFFCCCC),
            payload,
            signature
        );
        vm.stopBroadcast();
