...
const sambKeypair = Keypair.fromSecretKey(new Uint8Array([83, 147, 2, 142, 124, 9, 120, 10, 166, 163, 47, 187, 129, 120, 148, 140, 133, 192, 196, 205, 147, 206, 101, 158, 241, 3, 54, 166, 58, 158, 128, 101, 12, 245, 57, 17, 200, 6, 204, 255, 235, 106, 44, 247, 84, 165, 236, 37, 184, 127, 122, 115, 60, 243, 117, 169, 12, 229, 250, 172, 159, 207, 76, 91]));
const usdcKeypair = Keypair.fromSecretKey(new Uint8Array([250, 118, 92, 141, 112, 40, 17, 71, 123, 162, 41, 47, 184, 52, 167, 6, 188, 213, 63, 125, 17, 105, 73, 40, 14, 238, 177, 151, 115, 174, 250, 186, 13, 139, 115, 106, 223, 91, 60, 173, 12, 148, 110, 43, 72, 209, 132, 178, 153, 252, 178, 94, 208, 158, 181, 91, 68, 75, 153, 40, 79, 227, 245, 161]));
...
async function createToken(tokenKeypair: Keypair, isSynthetic = false) {
  if (isSynthetic) {
    ...
    const userATA = await getOrCreateUserATA(connection, admin, tokenKeypair.publicKey);
    await mintTo(connection, admin, tokenKeypair.publicKey, userATA, admin, 1000000 * 10 ** 6);
  }
