...
function newEd25519Instruction_(
  ...
  if (numSignatures !== pubkeys.length)
    throw new Error(`number of signatures and public keys must match`);

  // Calculate offsets
  const offsetsDataEnd =
  ...
  for (let i = 0; i < numSignatures; i++) {
    ...
    ED25519_INSTRUCTION_LAYOUT.encode(
      ...
      instructionData,
      SIGNATURE_OFFSETS_START + i * SIGNATURE_OFFSETS_SERIALIZED_SIZE
    );
  }

  signatures.forEach((sign, i) => {
    if (sign.length !== SIGNATURE_SERIALIZED_SIZE)
      throw new Error(
        `signature ${i} must be ${SIGNATURE_SERIALIZED_SIZE} bytes, got ${sign.length}`
      );
    const signatureOffset = signsOffset + i * SIGNATURE_SERIALIZED_SIZE;
  ...
  pubkeys.forEach((pk, i) => {
    if (pk.length !== PUBKEY_SERIALIZED_SIZE)
      throw new Error(
        `public key ${i} must be ${PUBKEY_SERIALIZED_SIZE} bytes, got ${pk.length}`
      );
    const publicKeyOffset = pksOffset + i * PUBKEY_SERIALIZED_SIZE;
    instructionData.fill(pk, publicKeyOffset);
  });

  if (message.length !== MESSAGE_SERIALIZED_SIZE)
    throw new Error(
      `message must be ${MESSAGE_SERIALIZED_SIZE} bytes, got ${message.length}`
    );
