...
export async function getBridgeTokenInfo(
  ...
  const [bridge_token_pda, _] = getBridgeTokenAccounts(
    token,
...
export function getUserNoncePda(user: PublicKey, bridgeProgramId: PublicKey) {
  const [nonceAccount] = PublicKey.findProgramAddressSync(
    [Buffer.from("nonce"), user.toBuffer()],
    bridgeProgramId
  );
  return nonceAccount;
}
...
export function getBridgeTokenAccounts(
  token: PublicKey,
  bridgeProgramId: PublicKey
) {
  const [pda] = PublicKey.findProgramAddressSync(
    [Buffer.from("token"), token.toBuffer()],
    bridgeProgramId
  );
...
export async function initializeToken(
  ...
  await bridgeProgram.methods
    .initializeToken([...hexToUint8Array(ambAddress)], ambDecimals, isSynthetic)
    .accountsPartial({
